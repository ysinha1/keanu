...
public class HalfCauchyVertex extends CauchyVertex {
    ...
    public Map<Vertex, DoubleTensor> dLogProb(DoubleTensor value, Set<? extends Vertex> withRespectTo) {
        Map<Vertex, DoubleTensor> cauchydLogProb = super.dLogProb(value, withRespectTo);
        if (value.greaterThanOrEqual(LOC_ZERO).allTrue()) {
            return cauchydLogProb;
        } else {
            for (Map.Entry<Vertex, DoubleTensor> entry : cauchydLogProb.entrySet()) {
                cauchydLogProb.put(entry.getKey(), DoubleTensor.create(0.0, entry.getValue().getShape()));
            }
            return cauchydLogProb;
        }
    }
